function tH = ecog_RenderGiftiLabelsPosNeg(g,vert_label,cmapInput,roiNames)
...
elseif isnumeric(cmapInput)
    cmapPos = cmapInput(size(cmapInput,1)/2+1:end,:);
    cmapNeg = cmapInput(1:size(cmapInput,1)/2,:);
    cmapNeg = cmapNeg(end:-1:1,:);
end

% convert surface labels into colors for vertices in mesh (c)
c = 0.7+zeros(size(vert_label,1),3);

% assign positive colors:
for k = 1:max(vert_label)
    c(ceil(vert_label)==k,:) = repmat(cmapPos(k,:),length(find(ceil(vert_label)==k)),1);
end

% assign negative colors:
for k = -1:-1:min(vert_label)
    c(ceil(vert_label)==k,:) = repmat(cmapNeg(-k,:),length(find(ceil(vert_label)==k)),1);
end
...
if ~isempty(roiNames)
    ...
    if iscell(roiNames)
        for k = 1:length(roiNames)
            plot(1,k,'.','Color',cmapInput(k,:),'MarkerSize',30)
            text(1.03,k,roiNames{k},'Color',cmapInput(k,:),'VerticalAlignment','middle')
        end
    elseif isnumeric(roiNames)
        for k = 1:length(roiNames)
            plot(1,k,'.','Color',cmapInput(k,:),'MarkerSize',20)
        end
